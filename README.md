...
│               ├── inspector/    Design / Styles / Code tabs, custom CSS import
│               ├── panels/       Widgets, layers, pages, theme
...
4. **Your own CSS** — in *Styles → Custom CSS for this widget* you can **import your own `.css` or
   ...
   registered in the exported `angular.json`.
5. **Component separation** — set *Extract as component* on any widget (or choose per-widget
...
6. **Export** — `Ctrl+E` opens the export drawer: choose the project name, component prefix,
...
│   └── app/
│       ├── app.config.ts         # provideRouter + zoneless-friendly setup
...
│       └── components/hero/      # extracted components, with signal inputs
...
`input()`s, `ngModel` bindings on form fields, `routerLink` on internal links and generated click
handlers for buttons. Exported projects compile (`ng build`) and their smoke specs pass
(`ng test`) — both are covered by this repo's checks.
...
- **`libs/schema`** — tree operations (insert/move/duplicate/cycle refusal), breakpoint style
  overrides, validation, migrations.
- **`libs/widgets`** — catalog integrity, derived styles, and the render plan contract.
- **`libs/generator`** — project scaffolding, lazy routes, component extraction, per-component
  custom CSS scoping, media queries, global stylesheet registration, Capacitor opt-in, JSON validity.
- **`apps/builder`** — the state store, plus a jsdom test that boots the whole shell and drives it:
  rendering the canvas, selecting a widget, editing a property, adding from the palette, opening
  the export drawer.
